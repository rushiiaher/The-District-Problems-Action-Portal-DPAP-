      ...
      <main className="flex-1 overflow-y-auto flex items-center justify-center p-8">
        <div className="max-w-md w-full text-center">
          ...
          <div className="flex gap-3">
            <Link href="/citizen/red-cross/status" className="flex-1 btn-navy py-2.5 text-center text-sm font-bold rounded">
              Track Status
            </Link>
            <Link href="/citizen/dashboard" className="flex-1 border border-slate-200 text-slate-600 py-2.5 text-center text-sm font-bold rounded hover:bg-slate-50">
              Dashboard
            </Link>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="h-14 flex items-center justify-between px-8 bg-red-700 text-white sticky top-0 z-10 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="w-6 h-6 relative flex-shrink-0">
              <div className="absolute inset-0 bg-white rounded-sm opacity-20" />
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-1.5 h-5 bg-white" />
                <div className="absolute w-5 h-1.5 bg-white" />
              </div>
            </div>
        ...
        <div className="p-8 max-w-3xl mx-auto">
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 relative flex-shrink-0">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  ...
                  <div className="absolute w-7 h-2 bg-white" />
                </div>
              </div>
              <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Section title="Personal Details" icon="person">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Field label="Aadhaar Number">
                  <input type="text" maxLength={12} value={form.aadhaar} onChange={e => set("aadhaar", e.target.value.replace(/\D/g, ""))}
                    placeholder="12-digit Aadhaar number" className={inputCls} />
                </Field>
                <Field label="Mobile Number" required>
                  <input type="tel" maxLength={10} value={form.mobile} onChange={e => set("mobile", e.target.value.replace(/\D/g, ""))}
            ...
            <Section title="Current Address" icon="home">
              ...
              <div className="mt-3">
                <LocationSelector
                  ...
                  onChange={({ district, block, village }) => setForm(f => ({ ...f, district, block, village }))}
                  required />
              </div>
            </Section>
            ...
            <Section title="Purpose of Aid" icon="volunteer_activism">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Field label="Amount Requested (₹)">
                  <input type="number" min="0" value={form.amount_requested} onChange={e => set("amount_requested", e.target.value)}
            ...
            {/* Documents */}
            <Section title="Supporting Documents" icon="attach_file">
              <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-red-400 transition-colors cursor-pointer"
                ...
                <p className="text-xs text-slate-400 mt-1">Aadhaar, Income Certificate, Medical Reports, Photos — PDF / JPG / PNG (max 5MB each, up to 5 files)</p>
                <input id="doc-upload" type="file" multiple accept=".pdf,.jpg,.jpeg,.png" className="hidden"
...
// ── Helpers ──────────────────────────────────────────────────────────────────
const inputCls = "w-full border border-slate-200 px-3 py-2.5 text-sm rounded focus:outline-none focus:border-red-400 text-slate-800 bg-white"
